...
`/v1/files` is implemented as a provider-neutral Polaris handle layer. The current surface supports project-scoped upload/list/get/delete/content with inline database storage and optional disk or S3-compatible blob backing. Blob-backed files are content-addressed by SHA-256, so byte-identical uploads share the same backing object while keeping independent Polaris ids and filenames. OpenAI, Anthropic, Google developer API, and Qwen register native `FilesAdapter` implementations, so `/v1/files/:id/materialize?provider=...` uploads bytes to the provider and caches the provider handle. Providers without a files API use image-only inline fallback where supported and otherwise return `provider_lacks_files_api` or a model capability error.

Polaris beta file understanding is separate from provider-native processing. It is disabled by default and only runs when operators enable `runtime.files.understanding` and callers opt in with `polaris.file_understanding`. In that path Polaris replaces unsupported file/image parts with derived text context and returns warning metadata because quality can be lower than native multimodal understanding.
...
- Scope: chat, image and embeddings
- Status: chat implemented; embeddings implemented; image generation/editing implemented
...
- Scope: video
- Status: Veo video implemented under the separate `google-vertex` provider name
...
- Scope: chat, image
- Status: chat implemented; image generation/editing implemented
...
- Scope: chat, image/voice, video, translation and simultaneous interpretation in the current speech wave
- Status: chat implemented; image generation/editing implemented; one-shot TTS and synchronous STT implemented in the voice wave; async Seedance video submit/poll/cancel implemented, expanded to the full planned request parity surface, and hardened with Polaris-owned content download; machine translation and simultaneous interpretation implemented on the current OpenSpeech speech stack
...
- Auth: `xi-api-key`
- Scope: music
- Status: music generation, streaming generation, composition plans, and stems implemented; preview for `v1.0.0` until explicitly opted into live smoke
