...
- You choose the key; only its hash is stored. Produce the `sha256:` hash with `openssl` (no Go toolchain needed) — see [AUTHENTICATION.md](AUTHENTICATION.md#keys-and-hashing) — or with `go run ./scripts/generate-key.go`. Commit only the hash, never the raw key.
...
- `runtime.auth.bootstrap_admin_key_hash` is required when `runtime.auth.mode: virtual_keys` and must be a `sha256:` hash of an admin key you choose (produce it with `openssl` per [AUTHENTICATION.md](AUTHENTICATION.md#keys-and-hashing), or `go run ./scripts/generate-key.go`); log into the console / call the admin API with the raw key
