...
// usageDimensionExpr maps a caller-facing grouping dimension to a safe SQL
// expression. The map is the ONLY allow-list — an unknown dimension is rejected,
// so the grouping key never comes from unsanitized input.
...
func (s *Store) ListAuditEvents(ctx context.Context, filter store.AuditFilter) ([]store.AuditEvent, error) {
	...
	if filter.Before != nil {
		add("created_at < ?", filter.Before.UTC())
	}
	...
	query = appendWhereExpression(query, strings.Join(clauses, " AND "))
