...
func (s *Store) ensureFilesUpgrade(ctx context.Context) error {
	...
	exists, err := s.sqliteColumnExists(ctx, "file_understanding_artifacts", "artifact_json")
	if err != nil {
		return err
	}
	if !exists {
		if _, err := s.db.ExecContext(ctx, `ALTER TABLE file_understanding_artifacts ADD COLUMN artifact_json TEXT NOT NULL DEFAULT '{}'`); err != nil {
			return err
		}
	}
	return nil
...
func (s *Store) GetFile(ctx context.Context, polarisID string) (*store.File, error) {
	row := s.db.QueryRowContext(ctx, fileSelectSQL()+` WHERE polaris_id = ? AND deleted_at IS NULL LIMIT 1`, polarisID)
...
func (s *Store) GetFileForProject(ctx context.Context, polarisID, projectID string) (*store.File, error) {
	row := s.db.QueryRowContext(ctx, fileSelectSQL()+` WHERE polaris_id = ? AND project_id = ? AND deleted_at IS NULL LIMIT 1`, polarisID, projectID)
	file, err := scanFile(row)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, store.ErrNotFound
...
func (s *Store) ListFiles(ctx context.Context, filter store.FileFilter) ([]store.File, error) {
	...
	if filter.AfterID != "" {
		clauses = append(clauses, "polaris_id < ?")
	...
	query += " ORDER BY polaris_id DESC LIMIT ?"
...
func (s *Store) DeleteFile(ctx context.Context, polarisID string) error {
	result, err := s.db.ExecContext(ctx, `DELETE FROM files WHERE polaris_id = ?`, polarisID)
	if err != nil {
...
func (s *Store) GetFileInline(ctx context.Context, polarisID string) ([]byte, error) {
	var data []byte
	err := s.db.QueryRowContext(ctx, `SELECT inline_bytes FROM files WHERE polaris_id = ? AND deleted_at IS NULL LIMIT 1`, polarisID).Scan(&data)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, store.ErrNotFound
		}
...
func (s *Store) DeleteFileProviderHandlesByPolarisID(ctx context.Context, polarisID string) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM file_provider_handles WHERE polaris_id = ?`, polarisID)
	if err != nil {
...
func (s *Store) PurgeExpiredFiles(ctx context.Context, now time.Time) (int64, error) {
	result, err := s.db.ExecContext(ctx, `DELETE FROM files WHERE expires_at IS NOT NULL AND expires_at < ?`, now.UTC())
	if err != nil {
...
func (s *Store) SumProjectFileBytes(ctx context.Context, projectID string) (int64, error) {
	...
	if err := s.db.QueryRowContext(ctx, `SELECT COALESCE(SUM(size), 0) FROM files WHERE project_id = ? AND deleted_at IS NULL`, projectID).Scan(&total); err != nil {
...
func (s *Store) CountProjectFiles(ctx context.Context, projectID string) (int64, error) {
	var total int64
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM files WHERE project_id = ? AND deleted_at IS NULL`, projectID).Scan(&total); err != nil {
...
func (s *Store) CountFilesByBlobKey(ctx context.Context, blobKey string) (int64, error) {
	var total int64
	if err := s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM files WHERE blob_key = ? AND deleted_at IS NULL`, blobKey).Scan(&total); err != nil {
...
func (s *Store) GetFileUnderstandingArtifact(ctx context.Context, sha256, processor, version string) (*store.FileUnderstandingArtifact, bool, error) {
	row := s.db.QueryRowContext(ctx, `
		SELECT sha256, processor, version, mime_type, text, warning, metadata_json, artifact_json, created_at
		FROM file_understanding_artifacts
		WHERE sha256 = ? AND processor = ? AND version = ?
		LIMIT 1
