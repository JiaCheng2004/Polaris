...
import (
	...
	"github.com/JiaCheng2004/Polaris/internal/provider/zaitoken"
)

func init() {
	registerProviderFamilyRegistrar("zai-token", registerZaiTokenProvider)
}

func registerZaiTokenProvider(registry *Registry, warnings *[]string, providerName string, providerCfg config.ProviderConfig) {
	client := zaitoken.NewClient(providerCfg)
	for modelName, modelCfg := range providerCfg.Models {
		...
		chatAdapter := zaitoken.NewChatAdapter(client, id, modelCfg.MaxOutputTokens)
		registry.chatAdapters[id] = chatAdapter
		registry.nativeMessagesAdapters[id] = zaitoken.NewNativeMessagesAdapter(client, id)
	}
}
