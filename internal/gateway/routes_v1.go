...
func registerV1CoreRoutes(v1 *gin.RouterGroup, handlers routeHandlers) {
	...
	v1.POST("/batches", handlers.idempotency("/v1/batches"), handlers.batches.Create)
...
func registerV1MediaRoutes(v1 *gin.RouterGroup, handlers routeHandlers) {
	...
	v1.POST("/music/generations", handlers.idempotency("/v1/music/generations"), handlers.music.Generate)
	...
	v1.POST("/video/generations", handlers.idempotency("/v1/video/generations"), handlers.video.Generate)
...
func registerV1VoiceAudioRoutes(v1 *gin.RouterGroup, handlers routeHandlers) {
	...
	v1.POST("/audio/notes", handlers.idempotency("/v1/audio/notes"), handlers.notes.Create)
	v1.GET("/audio/notes/:id", handlers.notes.Get)
	v1.DELETE("/audio/notes/:id", handlers.notes.Delete)
	v1.POST("/audio/podcasts", handlers.idempotency("/v1/audio/podcasts"), handlers.podcast.Create)
...
func registerV1ControlPlaneRoutes(v1 *gin.RouterGroup, deps Dependencies, handlers routeHandlers) {
	...
	control.GET("/admin/usage", handlers.adminAnalytics.Usage)
	control.GET("/admin/audit-events", handlers.adminAnalytics.AuditEvents)
